# 안녕하세요! 👋

저는 **Duong Nguyen**입니다. 소프트웨어 개발자로서 다양한 프로젝트와 기술에 관심이 많습니다. 제 프로필을 방문해주셔서 감사합니다!

## 저에 대해

- **이름**: Duong Nguyen
- **직업**: 소프트웨어 개발자
- **취미**: 코딩, 독서, 여행, 음악 감상

## 관심사

- **프로그래밍 언어**: JavaScript, Dart, Java
- **프레임워크**: React, Node.js, Flutter
- **기술**: 클라우드 컴퓨팅, 인공지능, 데이터 시각화

## 주요 프로젝트

### 1. [프로젝트 이름 1](https://github.com/duongnv12/financial_management)
간단한 설명: 이 프로젝트는 __을(를) 목표로 합니다. 주요 기능은 __ 입니다.

### 2. [프로젝트 이름 2](https://github.com/duongnv12/Compilers-Lab)
간단한 설명: 이 프로젝트는 __을(를) 목표로 합니다. 주요 기능은 __ 입니다.

### 3. [프로젝트 이름 3](https://github.com/duongnv12/project3)
간단한 설명: 이 프로젝트는 __을(를) 목표로 합니다. 주요 기능은 __ 입니다.

## 연락처

- **이메일**: [duong.04@outlook.com](mailto:duong.04@outlook.com)
- **GitHub**: [github.com/duongnv12](https://github.com/duongnv12)

제 저장소를 둘러보시고, 피드백이나 협업 제안이 있으시면 언제든지 연락주세요! 😊
